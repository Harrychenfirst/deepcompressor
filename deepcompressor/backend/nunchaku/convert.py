...
# Rename deepcompressor's block-sublayer param suffixes to the official
# NunchakuFlux2Transformer2DModel names. Shapes/values/semantics are identical
# (verified against SVDQW4A4Linear.forward): only the key strings differ.
_NUNCHAKU_FLUX2_PARAM_RENAME = {
    "lora_down": "proj_down",
    "lora_up": "proj_up",
...
def _rename_to_nunchaku_flux2_naming(
    converted: dict[str, torch.Tensor], output_name_map: dict[str, str]
) -> dict[str, torch.Tensor]:
    """Remap block-sublayer keys to the official ``NunchakuFlux2Transformer2DModel`` layout.

    - Module names: deepcompressor's short names (``qkv_proj``, ``out_proj``, ...) are
      mapped to the official module paths (``attn.to_qkv``, ``attn.to_out.0``, ...) via
      ``output_name_map``.
    - Param suffixes are renamed via :data:`_NUNCHAKU_FLUX2_PARAM_RENAME`
      (``lora_down/up`` -> ``proj_down/up``, ``smooth[_orig]`` -> ``smooth_factor[_orig]``).
    - ``bias`` is dropped: the official ``SVDQW4A4Linear`` layers are built biasless, so
      an emitted ``bias`` key would make ``load_state_dict`` fail on an unexpected key.
    """
    renamed: dict[str, torch.Tensor] = {}
    for key, value in converted.items():
        local_name, _, param = key.rpartition(".")
        if param == "bias":
            continue
        assert local_name in output_name_map, f"Unmapped Flux2 sublayer {local_name!r} (key {key!r})"
        new_key = f"{output_name_map[local_name]}.{_NUNCHAKU_FLUX2_PARAM_RENAME.get(param, param)}"
        assert new_key not in renamed, f"Duplicate key {new_key!r} after Flux2 rename"
...
def convert_to_nunchaku_flux2_single_transformer_block_state_dict(
    ...
    # Single blocks nest every sublayer under ``attn.`` in the official layout.
    return _rename_to_nunchaku_flux2_naming(
        converted,
        {
...
def convert_to_nunchaku_flux2_transformer_block_state_dict(
    ...
    converted = convert_to_nunchaku_transformer_block_state_dict(
        ...
        branch_name_map={
            "qkv_proj": "attn.to_q",
            # The fused context qkv low-rank branch is stored under add_q_proj (Q),
            # while its smooth factor lives under add_k_proj (K) — see branch.pt/smooth.pt.
            "qkv_proj_context": "attn.add_q_proj",
            "out_proj": "attn.to_out.0",
            "out_proj_context": "attn.to_add_out",
            "mlp_fc1": "ff.linear_in",
            "mlp_fc2": down_proj_local_name,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--flux2", action="store_true", default=None, help="use Flux2 block layout (auto-detected if not specified)."
    )
